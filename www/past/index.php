...
<?
    ...
    if($currYear=="2010") {
        $aryOffset = array(-1,5,1,1,4,6,2,4,0,3,5,1,3);
    }
    if($currYear=="2011") {
        $aryOffset = array(-1,6,2,2,5,0,3,5,1,4,6,2,4);
    }
    if($currYear=="2012") {
        $aryOffset = array(-1,0,3,4,0,2,5,0,3,6,1,4,6);
    }
	if($currYear=="2013") {
	$aryOffset = array(-1,2,5,5,1,3,6,1,4,0,2,5,0);
    }
    if($currYear=="2014") {
	$aryOffset = array(-1,3,6,6,2,4,0,2,5,1,3,6,1);
    }
    if($currYear=="2015") {
	$aryOffset = array(-1,4,0,0,3,5,1,3,6,2,4,0,2);
    }
    if($currYear=="2016") {
        ...
        function tag($name, $attrs, $children) {
            ...
            if(strlen($name)>0&&$name!="") {
                ...
                if($currenttag=="DATE" && $name!="STATUS") {
                    ...
                    //$finishTime = date("Y\-m\-d H:i:s", mktime($hour-5,$minute,$second,$month,$day,$year));

		    $thetime = strtotime($name);
		    $finishTime = date("Y\-m\-d h:i:s a", $thetime);
...
<p align=left>Enter only a single host name or IP address, or a list of host names and IP addresses,
             ...
             if($currNum<=$aryDays[(int)$currMonth]&&$currNum>0) {
                ...
                for($k=0;$k<$dataTotal;$k++) {
                    ...
                    if((int)$dataDay==(int)$currNum&&(int)$currMonth==(int)$dataMonth) {
                        ...
                        $detailString = "IP: " . $aryData[$k][0] . "<br>Started: ". $aryData[$k][1] . " CST<br>Type: " . $aryData[$k][3] . "<br>Title: " . $aryData[$k][4] ;
                        print "<table><tr><td align=center onmouseover=\"tooltip.show('" . $detailString . "')\" class=element bgcolor=" . $cellColor . " onmouseout=\"tooltip.hide();\"><font color='" . $fontColor . "'> " . " Scan found. Hover over for more info.</font><img src=../images/scanned.png height=50 /></td></tr></table>";
                    }
                }
