...
<?
    $url = "https://qualysapi.qg2.apps.qualys.com/api/2.0/fo/schedule/scan/?action=list&active=1"; // Qualys API v2 only running scans
    $error = "";
    $aryData = array();
   ...
   class tag {
	    ...
	    # capture v2 next launch date
	    if($currenttag=="NEXTLAUNCH_UTC"&&$name!=""&&$name!="TIME_ZONE"){
		$timestamp = strtotime($name);
		$aryData[$i][0] = date("m-d h:i:s a", $timestamp)." CST";
	    }

	    # catpure v2 profile
	    if($currenttag=="TITLE"&&$name!=""&&$name!="USER_LOGIN"&&$name!="DEFAULT_FLAG"){
		if($aryData[$i][4]==""){
			$aryData[$i][4] = $name;		
		}
	    }
	    ...
	    # capture v2 status
	    if($currenttag=="ACTIVE"&&$name!=""&&$name!="TITLE"){
		if($name==1) { $aryData[$i][3] = "ACTIVE"; }
	    }
 
	    # capture v2 targets 
	    if($currenttag=="TARGET"&&$name!=""&&$name!="ISCANNER_NAME"){
		$aryData[$i][2] = $name;
	    }

	    # capture end of record
	    if($currenttag=="DST_SELECTED"&&$name!=""&&$name!="SCAN"){
		$i++;
	    }
        }
    }
